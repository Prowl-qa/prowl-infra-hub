...
**Resolved**: 2026-06-04 (branch `terraform-driver`)
**Description**: Kept scheduled `--all --driver ec2` runs scoped to Ansible playbooks by filtering the all-run queue before EC2 harness setup, while preserving explicit single Terraform playbook execution under `--driver ec2` as local plan-mode. Extended `cli/drivers/terraform.ts` to parse required first-level attributes from Terraform `object({ ... })` variable constraints, skip optional object attributes, and synthesize typed object stubs in both `terraform.tfvars` and unquoted HCL placeholder rendering. Added regression coverage for inline and multi-line object constraints, optional object attributes, and generated object tfvars. Verified `./node_modules/.bin/eslint .`, `npm run typecheck`, `node --experimental-strip-types --test tests/playbooks.test.ts`, and `node --experimental-strip-types --test tests/terraform-driver.test.ts`.
