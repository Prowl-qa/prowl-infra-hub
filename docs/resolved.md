...
## ~~INFRA-067: Resolve PR #34 CLI publish workflow review findings~~
**Resolved**: 2026-06-05 (branch `add-cli-publish-workflow`)
**Description**: Hardened `.github/workflows/publish-cli.yml` after PR #34 review. The manual npm publish job now runs only when dispatched from `main`, pins `actions/checkout` and `actions/setup-node` to commit SHAs, disables checkout credential persistence, and runs `npm run typecheck` inside `cli/` before the `npm publish --ignore-scripts` step. The CodeRabbit trusted-publishing suggestion was kept as a separate follow-up because switching away from `NPM_TOKEN` requires npm package-side trusted publisher configuration before the workflow can safely rely on GitHub OIDC. Verified `./node_modules/.bin/eslint .`, `npm run typecheck`, `node --experimental-strip-types --test tests/playbooks.test.ts`, `npm --prefix cli run typecheck`, and `npm run build:cli`; `actionlint` was not installed locally.
...
## ~~INFRA-070: Tear down the AWS EC2 test environment and revoke cloud access~~
**Resolved**: 2026-08-26 (completed: 2026-08-26; branch `sunset-infra-hub`)
**Description**: First item of the sunset (see the backlog's "Sunset Work Items"). `terraform destroy` in `infra/ec2-test-env/` removed all 10 managed resources — VPC, subnet, internet gateway, route table + association, security group, `prowl-molecule-key` key pair, the GitHub Actions OIDC provider, and the `prowl-molecule-gh-actions` IAM role with its inline EC2/spot policy. Verified afterwards via AWS CLI (account `278635088908`, `us-east-1`): no `Project`-tagged resources, no OIDC providers, no `prowl*`/`molecule*` roles, no key pairs, and no instances, spot requests, or RDS in any US/EU region; Terraform state is empty. All seven repo secrets (`AWS_ROLE_ARN`, `EC2_KEY_PAIR_NAME`, `EC2_SECURITY_GROUP_ID`, `EC2_SSH_PRIVATE_KEY`, `EC2_SUBNET_ID`, plus `NPM_TOKEN` and `CLAUDE_CODE_OAUTH_TOKEN` for INFRA-071/073) deleted; the repo had no Actions variables. `test-aws-connection.yml` and `test-playbook-ec2.yml` (weekly cron, failing since July) removed. Correction to the item as written: `terraform.tfstate` and `terraform.tfvars` were never committed (`*.tfstate`/`*.tfvars` are gitignored — only `main.tf`, `outputs.tf`, `variables.tf`, and the lock file are tracked), and the local tfvars holds only an ED25519 public key, so there was nothing sensitive in public history. The Terraform files stay in the repo as a historical reference; `docs/ec2-testing.md` and the "Established Architectural Decisions" section of `CLAUDE.md` are marked historical.

## ~~INFRA-071: Deprecate the `prowl-infra` npm package~~
**Resolved**: 2026-08-26 (completed: 2026-08-26; branch `sunset-infra-hub`)
**Description**: `prowl-infra@0.0.1` (the only published version) deprecated via the npmjs.com package settings UI; `npm view prowl-infra deprecated` now returns npm's stock notice ("Package no longer supported. Contact Support at https://www.npmjs.com/support for more info."). The package stays installable but unmaintained; `cli/README.md` carries the retirement banner pointing at prowl.tools and the Prowl CLI. `publish-cli.yml` removed and the `NPM_TOKEN` repo secret deleted, so no publish path remains. Closes INFRA-068 (npm trusted publishing) as won't-do.

## ~~INFRA-073: Remove automation attached to this repo~~
**Resolved**: 2026-08-26 (completed: 2026-08-26; branch `sunset-infra-hub`)
**Description**: All nine workflow files deleted (`claude-code-review.yml`, `claude.yml`, `lint-workflows.yml`, `publish-cli.yml`, `sync-to-database.yml`, `test-aws-connection.yml`, `test-playbook-ec2.yml`, `test-playbook.yml`, `validate-submission.yml`; the `prowl-review*.yml` pair only ever lived on the unmerged `prowl-review-codex` branch). Self-hosted runner `lucius-mac-mini-prowl-infra-hub` (id 21) deregistered via the GitHub API (repo now lists zero runners), then on the Mac mini its launchd service `actions.runner.prowl-tools-prowl-infra-hub.lucius-mac-mini-prowl-infra-hub` was uninstalled with `svc.sh uninstall`, and the install directory + logs removed (the listener had already deleted its own `.runner`/`.credentials` on seeing the server-side removal); the mini's other seven runners were untouched. Remote `prowl-review-codex` branch deleted. `CLAUDE_CODE_OAUTH_TOKEN` secret deleted (all repo secrets are gone as of INFRA-070/071). **Caveat on the "no GitHub Apps installed" acceptance:** CodeRabbit, Claude, and prowl-review are installed at the `prowl-tools` org level with "all repositories", so there is no per-repo uninstall; archiving the repo (INFRA-074) makes them inert here, and narrowing the org installations is a separate owner decision outside this repo.

## ~~INFRA-072: Decommission the site, database, and DNS~~
**Resolved**: 2026-08-26 (completed: 2026-08-26; branch `sunset-infra-hub`)
**Description**: Vercel: the Neon Postgres Marketplace resource `prowl-infra-hub-db` was disconnected and deleted (`vercel integration-resource remove --disconnect-all`; the Upstash Redis resource from INFRA-057 was already uninstalled), then the `prowl-infra-hub` Vercel project was removed with all deployments and its domain binding — `https://infra.prowl.tools` returned Vercel 404 immediately afterwards. `sync-to-database.yml` deleted; `DATABASE_URL` had never been set as a repo secret (INFRA-054), so no other database was ever wired up and nothing is left billing. Owner removed the `infra` CNAME (→ `*.vercel-dns-017.com`) from the Cloudflare `prowl.tools` zone; verified via the authoritative nameserver (`@paris.ns.cloudflare.com`) that `infra.prowl.tools` no longer resolves. The Drizzle schema/migrations, `scripts/seed.ts`, and `infrastructure/prowl-hub-postgres/` stay in the repo as historical reference only.
