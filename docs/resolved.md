...
**Description**: Added `cli/drivers/terraform.ts` implementing plan-mode testing: `terraform init -backend=false` + `terraform validate` (both required to pass) + best-effort `terraform plan` for provider auth/API failures (captured in the report's `terraform.planError` field), while non-auth plan errors now fail the test so missing variables and malformed catalog configuration are not reported as passed. Driver extracts the HCL block from the YAML `playbook: |` field via `extractTerraformBlock` in `cli/drivers/terraform.ts`, mirroring the YAML block extraction approach used by `extractPlaybookBlock` in `cli/drivers/ansible-ec2.ts`, dedents to remove the `playbook: |` indent, strips the leading `---` YAML document separator (the HCL is otherwise valid). Reads the YAML `vars:` section and writes a stub `terraform.tfvars` with generic string values for each declared variable to satisfy required-without-default variables during plan. Stubs major-provider credentials (AWS / Azure / GCP) in `STUB_PROVIDER_ENV` so plan can attempt provider init even though it will inevitably fail at the cloud API. Wired into `cli/index.ts` for both `--driver docker` and `--driver ec2` paths — terraform plan-mode is identical regardless of `--driver` because no real resources are deployed. New `tests/terraform-driver.test.ts` covers `extractTerraformBlock` and `extractDeclaredVarNames` (6 unit tests). End-to-end smoke against all 10 Terraform playbooks in the catalog: 8 PASSED (validate clean, plan errored at auth as expected), 2 FAILED with legitimate catalog bugs — `provisioning/gcp-project-setup.yml` has a provider/resource cycle (`google_project.main` referenced inside `provider "google"`), and `monitoring/cloudwatch-alarms.yml` fails at init-level. Apply-mode upgrade evaluation captured as PQIH-025 / INFRA-061. Catalog now has CLI test coverage for 32 of 38 playbooks (84%) — was 22/38 (58%) pre-merge.
